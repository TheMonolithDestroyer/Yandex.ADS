...
namespace Yandex.Practicum.Sprints.Sprint2
{
    public class TaskFinalA : BaseClass
    {
        ...
        // ВРЕМЕННАЯ СЛОЖНОСТЬ
        //  Перебор всех команд в цикле - O(N), где N - количество раз, в котором цикл должен быть выполнен(0 < commands).
        //  Дек фиксированного размера и использован принцип кольцевого буфера, то добавление и удаление занимает O(1) константное время.
        //  В О - нотации не учитываются константы и коэффиценты, получается, итого временная сложность алгоритма - O(N), где N - это количество команд(commands).

        // ПРОСТРАНСТВЕННАЯ СЛОЖНОСТЬ
        // Space complexity - O(K), где K - колличество элементов(команд);
